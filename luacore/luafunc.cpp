...
static const struct SLuaConst const_data[] = {
	{ "CONST_VK_BACK",     0x08 },
	{ "CONST_VK_TAB",     0x09 },
	{ "CONST_VK_CLEAR",     0x0C },
...
void CLuaCore::registerConst() {
	if (NULL == m_LuaState) return;

	for (int i=0; NULL != const_data[i].name; i++) {
		lua_pushnumber(m_LuaState, const_data[i].value);
		lua_setglobal(m_LuaState, const_data[i].name);
	}
}
